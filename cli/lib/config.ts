...
const ANVIL_DEFAULTS = {
  ...
  deploymentPrivateKey: "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80",
...
export function loadConfig(): Partial<Config> {
  ...
  const config: Partial<Config> = {
    ...
    chainId: env.CHAIN_ID ? parseInt(stripQuotes(env.CHAIN_ID), 10) : ANVIL_DEFAULTS.chainId,
    networkName: stripQuotes(env.NETWORK_NAME) || ANVIL_DEFAULTS.networkName,

    // Deployment (with Anvil defaults)
    deploymentPrivateKey: stripQuotes(env.DEPLOYMENT_PRIVATE_KEY) || ANVIL_DEFAULTS.deploymentPrivateKey,
    ...
    verifier: (stripQuotes(env.VERIFIER) || ANVIL_DEFAULTS.verifier) as VerifierType,
    ...
    daoFactoryAddress: stripQuotes(env.DAO_FACTORY_ADDRESS) || ARAGON_MAINNET.daoFactory,
    pluginRepoFactoryAddress: stripQuotes(env.PLUGIN_REPO_FACTORY_ADDRESS) || ARAGON_MAINNET.pluginRepoFactory,
    pluginSetupProcessorAddress: stripQuotes(env.PLUGIN_SETUP_PROCESSOR_ADDRESS) || ARAGON_MAINNET.pluginSetupProcessor,

    // Plugin (defaults to deployer address for local dev)
    pluginEnsSubdomain: stripQuotes(env.PLUGIN_ENS_SUBDOMAIN) || "my-plugin",
    pluginRepoMaintainerAddress: stripQuotes(env.PLUGIN_REPO_MAINTAINER_ADDRESS) || ANVIL_DEFAULTS.deploymentAddress,
