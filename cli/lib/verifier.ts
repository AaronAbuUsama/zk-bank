...
import type { VerifierType, BroadcastJson, VerifyOptions } from "../types";
...
export function getVerifierUrl(config: VerifierConfig): string | undefined {
  switch (config.type) {
    ...
    case "sourcify":
      return undefined; // Sourcify doesn't need URL
...
export async function verifyFromBroadcast(
  ...
  for (const contract of contracts) {
    ...
    if (!sourceFile) {
      console.log(`  Warning: Could not find source file for ${contract.contractName}`);
      continue;
    }
